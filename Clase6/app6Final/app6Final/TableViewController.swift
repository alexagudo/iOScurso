...
class TableViewController: UITableViewController {
    ...
    @IBAction func agregarPalabras(_ sender: Any) {
        let alerta = UIAlertController(title: "Nueva Palabra", message: "Agrega nueva palabra", preferredStyle: .alert)
        let guardar = UIAlertAction(title: "Agregar", style: .default, handler: {(UIAlertAction) -> Void in
            let textField = alerta.textFields!.first
            self.guardarPalabras(palabra: textField!.text!)
    ...
    func guardarPalabras(palabra:String){
        // A
        let appDelegate = UIApplication.shared.delegate as? AppDelegate
        let managedContext = appDelegate!.persistentContainer.viewContext
